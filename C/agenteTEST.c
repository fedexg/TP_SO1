...
void print_fields(char **fields, int len){
    for(int i = 0 ; i < len ; i++)
        printf("%s ",fields[i]);
    puts("OK.");
}
...
{
    for (;;) {
        // Devuelve la cantidad de eventos recientemente sucedidos en los sockets que epoll gestiona.
        ...
        // Comienza a leer los eventos sucedidos
        for (int i = 0; i < num_fds_ready; ++i) {
            //Si reccionó el lsock es porque un socket quiere conectarse a este agente C
            if (events[i].data.fd == listen_public_sock) { 
                ...
                //Seteamos el socket como no bloqueante
            ...
            // Si reaccionó el lsocket destinado a erlang, es porque
            // erlang quiere comenzar una conexion
                ...
                if (add_descriptor(connect_erlang_sock) < 0)
                    exit(EXIT_FAILURE);
            // si es una conexion ya comenzada, verifica a
            // qué socket pertenece y atiende su peticion.
            } else {
...
void handle_erlang_client(int fd)
{
    ...
    if (bytes_read <= 0)
        ...
        int length = 0;
        char **request_fields = split(buffer, " ", &length);
        print_fields(request_fields, length);
