#include <assert.h>
#include <arpa/inet.h>
#include <errno.h>
#include <stdio.h>
#include <sys/socket.h>
#include <sys/epoll.h>
#include <sys/un.h>
#include <string.h>
#include <unistd.h>
#include <fcntl.h>
#include <pthread.h>

#define OK    0
#define FAIL  -1
#define PORT  12529
#define MAX_EVENTS 10 //<- raro...

// SOCK_STREAM -> TCP
// SOCK_DGRAM ->  UDP

int public_socket, listen_public_socket, erlang_sock;
int epoll_fd;
pthread_t epoll_thread;
struct epoll_event ev, events[MAX_EVENTS];

void error(const char *msg);
int init_sockets(void);
int init_agents_socket(void);
int init_erlang_socket(void);
int init_epoll(void);
int startup(void);
int add_descriptors(void);
int close_epoll(void);
int close_sockets(void);

int main()
{
    if (init_sockets() < 0)
        return 1;

    if (init_epoll() < 0) {
        int rc = close_sockets();
        if (rc < 0)
            return 1;

        return 1;
    }

    if (startup() < 0) {
        int rc = close_sockets();
        if (rc < 0)
            return 1;

        return 1;
    }

    if(add_to_epoll() < 0){
        int rc = close_sockets();
        if (rc < 0)
            return 1;

        return 1;
    }

    if (add_descriptors()) {
        int rc = close_sockets();
        if (rc < 0)
            return 1;

        return 1;
    }

    for (;;) {
        // TODO
    }

    if (close_epoll() < 0)
        return 1;

    if (close_sockets() < 0)
        return 1;

    return 0;
}

void error(const char *msg)
{
    printf("%s: %s\n", msg, strerror(errno));
}

int init_sockets(void)
{
    if (init_agents_socket() < 0)
        return FAIL;

    if (init_erlang_socket() < 0)
        return FAIL;

    return OK;
}

int init_agents_socket(void)
{
    int yes = 1;
    public_socket = socket(AF_INET, SOCK_STREAM, 0);
    if (public_socket < 0) {
        error("Error intentando iniciar el socket para otros agentes");
        return FAIL;
    }

    // Set agent sockets to be reused
    if (setsockopt(public_socket, SOL_SOCKET, SO_REUSEADDR, &yes, sizeof(yes)) == 1) {
        error("No se pudo configurar el socket público para ser reusado");
        return FAIL;
    }

    // Bind and set socket to listen mode
    struct sockaddr_in sa_public;
    sa_public.sin_family = AF_INET;
    sa_public.sin_port = htons(PORT);
    sa_public.sin_addr.s_addr = htonl(INADDR_ANY);

    if (bind(public_socket, (struct sockaddr *)&sa_public, sizeof(sa_public)) < 0) {
        error("Error intentando asignar una dirección al socket público");
        return FAIL;
    }

    if (listen(public_socket, 10) < 0) {
        error("Error intentando setear el socket público para escuchar");
        return FAIL;
    }

    return OK;
}

int init_erlang_socket(void)
{
    int yes = 1;
    erlang_sock = socket(AF_UNIX, SOCK_STREAM, 0);
    if (erlang_sock < 0) {
        error("Error intentando iniciar el socket para Erlang");
        return FAIL;
    }

    // Set erlang sockets to be reused
    if (setsockopt(erlang_sock, SOL_SOCKET, SO_REUSEADDR, &yes, sizeof(yes)) == 1) {
        error("No se pudo configurar el socket de Erlang para ser reusado");
        return FAIL;
    }

    // Bind and set socket to listen mode
    struct sockaddr_un sa_erlang;
    sa_erlang.sun_family = AF_UNIX;
    const char *socket_path = "/tmp/erlang_socket";
    strncpy(sa_erlang.sun_path, socket_path, strlen(socket_path) - 1);

    // In case the socket was left over from a previous execution, delete it
    unlink(sa_erlang.sun_path);

    if (bind(erlang_sock, (struct sockaddr *)&sa_erlang, sizeof(sa_erlang)) < 0) {
        error("Error intentando asignar una dirección al socket de Erlang");
        return FAIL;
    }

    if (listen(erlang_sock, 10) < 0) {
        error("Error intentando setear el socket Erlang para escuchar");
        return FAIL;
    }

    return OK;
}

int init_epoll(void)
{
    epoll_fd = epoll_create(1);
    if (epoll_fd < 0) {
        error("Error intentando crear la instancia de epoll");
        return FAIL;
    }

    return OK;
}

int add_to_epoll()
{
    ev.events = EPOLLIN;
    ev.data.fd = listen_public_socket;
    if (epoll_ctl(epoll_fd, EPOLL_CTL_ADD, listen_public_socket, &ev) == -1) {
        perror("epoll_ctl: listen_public_socket");
        return FAIL;
    }
    return OK;
}

int init_handler_epoll()
{
    if(pthread_create(&epoll_thread, NULL, handler_epoll, NULL) < 0)
        return FAIL;
    return OK;
}

void *handler_epoll(void *){
    for (;;) {
        int nfds = epoll_wait(epoll_fd, events, MAX_EVENTS, -1);
        if (nfds == -1) {
            perror("epoll_wait");
            exit(1);
        }

        for (int n = 0; n < nfds; ++n) {
            if (events[n].data.fd == listen_public_socket) {
                public_socket = accept(listen_public_socket,
                                    (struct sockaddr *) &addr, &addrlen);
                if (public_socket == -1) {
                    perror("accept");
                    exit(1);
                }
                setnonblocking(public_socket);
                ev.events = EPOLLIN | EPOLLET;
                ev.data.fd = public_socket;
                if (epoll_ctl(epoll_fd, EPOLL_CTL_ADD, public_socket,
                            &ev) == -1) {
                    perror("epoll_ctl: public_socket");
                    exit(1);
                }
            } else {
                do_use_fd(events[n].data.fd);
            }
        }
    }
}

int startup(void)
{
    assert(0 && "TODO: startup not implemented");
    return OK;
}

int add_descriptors(void)
{
    assert(0 && "TODO: add_descriptors not implemented");
    return OK;
}

int close_epoll(void)
{
    if (close(epoll_fd) < 0) {
        error("Error intentando destruir la instancia de epoll");
        return FAIL;
    }

    return OK;
}

int close_sockets(void)
{
    if (close(erlang_sock) < 0) {
        error("Error intentando cerrar el socket de Erlang");
        return FAIL;
    }

    if (close(public_socket) < 0) {
        error("Error intentando cerrar el socket para los agentes");
        return FAIL;
    }

    return OK;
}
