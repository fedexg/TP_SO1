-module(scheduler_for_script).
-export([start_from_makefile/1]).

-export([start_scheduler/1, job_manager/5, message_manager/3]).

% 
% %
start_from_makefile([PuertoStr]) ->
    %% Convertimos el string "1234" a entero 1234
    Puerto = list_to_integer(atom_to_list(PuertoStr)), 
    start(Puerto).

% Inicia el iniciador del Scheduler y el simulador de cliente 
% %
start(PORT) ->
    Scheduler = spawn(?MODULE, start_scheduler, [PORT]),
